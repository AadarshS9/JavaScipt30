...
const inventors = [
  ...
  const filtered = inventors.filter(inventors => inventors.year >=1500 && inventors.year < 1600);
  console.table(filtered);

  // Array.prototype.map()
  // 2. Give us an array of the inventors first and last names
  const mapped = inventors.map(inventors => inventors.first + ' ' + inventors.last);
  console.log(mapped);

  // Array.prototype.sort()
  // 3. Sort the inventors by birthdate, oldest to youngest
  const sortedByDate = inventors.sort((first,second) => first.year > second.year ? 1 : -1);
  ...
  const sortedByYears = inventors.sort((first,second) => {
  ...
  (function (window) {
      ...
      var wikiCategory = document.querySelector('.mw-category');
      var links = wikiCategory.querySelectorAll('a');
      var boulevards;

      list = Array.from(links);

      boulevards = list.map(function (link) {
